...
pub fn init(lua: &Lua) -> crate::Result<()> {
    lua.context(|lua| {
        ...
        module.set("open", lua.create_function(|_, (path, mode): (String, Option<String>)|{
        ...
        module.set("read_dir", lua.create_function(|lua, path: String| {
            ...
            let list_value: serde_json::Value = serde_json::to_value(_list).map_err(LuaError::external)?;
            ...
            match fs::metadata(path) {
                Ok(md) => {
                    ...
                    table.set("type", {
                        ...
                        if file_type.is_file() { "file" } else if file_type.is_dir() { "directory" } else { unreachable!() }
                    })?;
